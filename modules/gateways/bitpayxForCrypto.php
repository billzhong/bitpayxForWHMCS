...
{
    return [
        "FriendlyName" => ["Type" => "System", "Value" => "微信或数字货币"],
...
function bitpayxForCrypto_link($params)
{
    ...
    if (!stristr($_SERVER['PHP_SELF'], 'viewinvoice')) {
        return '<img style="width: 150px" src="' . $systemurl . 'modules/gateways/bitpayx/crypto.png" alt="微信或数字货币支付" />';
    }
    ...
    if (($result['status'] === 200 || $result['status'] === 201) && $result['payment_url']) {
        ...
        $code_ajax = '<a href="' . $webpaylink . '" target="_blank" id="cryptoBitpayX" class="btn btn-info btn-block">前往支付页面</a>';
        ...
        if ($result['order']['status'] === 'NEW') {
            ...
            $webpaylink = 'https://invoice.mugglepay.com/invoices/?id=' . $result['order']['order_id'] . '&lang=zh';
            $code_ajax = '<a href="' . $webpaylink . '" target="_blank" id="cryptoBitpayX" class="btn btn-info btn-block">支付成功，等待确认</a>';
        }
    } else {
